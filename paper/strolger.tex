...
\begin{equation}
k = \frac{\int\limits_{3M_{\odot}}^{8M_{\odot}} \xi(M)\,dM}{\int\limits_{0.1M_{\odot}}^{125M_{\odot}} M\,\xi(M)\,dM}= 0.021^{+52\%}_{-38\%}\,M_{\odot}^{-1},
\end{equation}}
\noindent where error in $k$ is driven more by choices in the upper and lower value in the selected mass range of SN~Ia progenitors than by the choice in IMF model, as detailed above. \textcolor{red}{The error shown are 

\authorcomment3{The preceding three paragraphs address referee comment 3, on the uncertainties in $k$ derived from the mass ranges of expected CO WD progenitor stars.}
