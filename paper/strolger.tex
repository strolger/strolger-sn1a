...
\end{equation}
\noindent $R_{{\rm Ia},i}$ are the various independent rate measures, and $R_{\rm Ia}(t_i)$ are the parameter-dependent model predictions at the cosmic time of the various rate measures. We then find the optimal parameters which maximize this likelihood. \authorcomment2{These changes address reviewer comment 5. The factor f is a free parameter that accounts for the underestimation of uncertainties described above.  By allowing just a single f value applied to all rates measurements at all redshifts, we are effectively asserting that the scale of systematic ``uncertainty error'' is essentially Poisson and similar in all surveys.  That should be an acceptable assumption at least to first order.}

%\textcolor{red}{we assume that the uncertainties for all published volumetric rate measurements ($\sigma_i$) are gaussian in nature, but are underestimated by an amount that scales with the observed rates.  This is motivated by the fact that we are using just the statistical error reported for each rate value, and the most plausible sources of systematic uncertainty (such as classification errors and misestimated detection efficiencies) will tend to increase as the observed rate increases, and vice versa.}
