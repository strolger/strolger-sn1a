...
\begin{document}
\title{Continuous Empirical Delay Time Distributions of Type Ia Supernovae From Galaxy and Cosmic Star Formation Histories}
\author[0000-0002-7756-4440]{L.-G.~Strolger}
\author{Steven Rodney}
\author{Camila Pacifici}
\author{Gautham Narajan}
\author{Or Graur}
\author{et al.}
...
%\textcolor{red}{[Note: much of this is more an introduction to volumetric rates. Need more of an introduction to measures of host rates.]} 
...
% \textcolor{red}{I should at least talk about Parallel-Tempering Ensemble MCMC, and perhaps Dynamic Nested Sampling}
...
A common method to testing progenitor models is by comparing the measured rate of SNe~Ia in high specific star formation rate (sSFR; star-formation rate over the total stellar mass at the observed epoch) galaxies to that in their low-sSFR counterparts, in the modern `A+B'-model tests~\citep{Scannapieco:2005, Andersen:2018dp}. While this grossly addresses the promptness of some fraction of of SNe~Ia, the test is inherently flawed as it incorrectly assumes the observed SN~Ia rate is directly tied to the (A) total mass and (B) the current rate of star formation in a host galaxy, rather than appropriately connecting that SN rate to some past epoch of star formation. That, and the large uncertainties in SN rates that result from complex star-formation rate histories, are the largest sources of error in these tests in normal galaxies. 

Using the methods described in Section~\ref{sec:sfh} to convolve, for each SN~Ia host, the SFH by the best derived delay time distribution to get the SN rate at the observed epoch, and using catalog SFR and total masses at the observed epoch, we derive a track (and associated uncertainty region) in which SNe~Ia should lie in specific SN rates (sSNR) as a function of sSFR, shown in Figure~\ref{fig:ssfr}. This method is inherently more direct than that done in \cite{Graur:2015fk}, where in the latter the SFHs are estimated by an exponential law, following \cite{Gallazzi:2005rf} and
\cite{Kauffmann:2003sj}, and the references therein. This is shown in comparison to measurements from \cite{Mannucci:2005}, \cite{Sullivan:2006a}, and \cite{Smith:2012lr}, and in comparison to tracks expected from $\beta\approx-1.1$ power-law delay time models, and a piecewise model from \cite{Andersen:2018dp}. The measurements are consistent with all three tracks in over the region in which we have been able to measure them, for star-forming and `burst' galaxies. However, the tracks strongly diverge in the region of passive galaxies, where sSFR $\approx0$, where we only have upper limits to constrain them. It is expected that further studies in passive galaxies will provide some clarity. Additionally, tests of these tracks in field dwarf galaxies (not associated with galaxy clusters) may be illuminating as they are simpler to model by virtue of having many fewer episodes of star formation. 

\begin{figure}[t] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=6.5in]{figure_ssfr}
   \caption{\footnotesize }
   \label{fig:ssfr}
\end{figure}

As a final note, now that the evidence for double white dwarf mergers as the primary source of SNe~Ia reaches a fever pitch, it is increasingly interesting to investigate the fine details of the exact progenitor mechanism. 
...
\begin{figure}[t] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=6.5in]{figure_toonen}
   \caption{\footnotesize }
   \label{fig:toonen}
\end{figure}
...
\begin{figure}[h] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.0in]{figure_sfd_corners} 
...
\begin{figure}[h] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=5.0in]{figure_sfh_corners} 
