...
if __name__=='__main__':
    ...
    perr = sqrt(diag(pcov))
    ax.plot(xx, line_fn(xx, *popt), 'b-', label=r'Strolger et al. (2020)')
    ps = np.random.multivariate_normal(popt, pcov, 10000)
    ysample = asarray([line_fn(xx, *pi) for pi in ps])
    lower = percentile(ysample, 15.9, axis=0)
    upper = percentile(ysample, 84.1, axis=0)
    ...
    ax2.errorbar(data[1:,0], log10(data[1:,1]),
                 #yerr=[log10(data[1:,2]),log10(data[1:,3])],
                 yerr=-log10((data[1:,1]-data[1:,2])/data[1:,1]),
                 fmt='o', ms=10, mfc='white',  color='#2077B4', label='Mannucci et al. (2005)')
    ...
    ax2.errorbar(data[1:,0], log10(data[1:,1]),
                 yerr=-log10((data[1:,1]-data[1:,2])/data[1:,1]),
                 fmt='o', ms=10, mfc='white', color='#FF7F0E', label='Sullivan et al. (2006)')
    ax2.errorbar(data[0,0], log10(data[0,1]),
                 #yerr=-log10((data[0,1]-data[0,2])/data[0,1]),
                 yerr=0.5,
                 #xerr=[[data[0,0]+11.5],[data[0,0]-11.5]],
    ...
    ax2.errorbar(data[:,0], log10(data[:,1]),
                 ...
                 #uplims=lolims,
                 fmt='o', ms=10, mfc='white', color='#2CA02C', label='Smith et al. (2012)')
    ax2.errorbar(data[0,0], log10(data[0,1]),
                 #yerr=-log10((data[0,1]-data[0,2])/data[0,1]),
                 yerr=0.5,
                 #xerr=[[data[0,0]+11.5],[data[0,0]-11.5]],
    ...
    ## for shading upper and lower regions with little data
    ## tx = arange(-13,-10, 1) 
