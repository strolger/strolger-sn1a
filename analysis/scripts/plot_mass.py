...
if __name__=='__main__':
    ...
    fig.text(0.06,0.5,r'Average Mass ($M_{\odot}$)',ha='center',va='center',rotation='vertical')
    ...
    for metal in mass:
        ...
        data = data[ii]
        composite+=data[:,2]
        ax1.plot(data[:,0], log10(data[:,2]), '-', color='0.65',alpha=0.3)
    ax1.plot(data[:,0],log10(composite/len(mass)),'k-', label='%d SNe Hosts' %len(mass))
    ...
    for ia in ias:
        ...
        ii = where((data[:,0]>lbl)&(data[:,0]<=lbu))
        data = data[ii]
        goods_composite_ia += data[:,2]
    norm1=len(ias)-misses
    ax3 = subplot(222)
    ax31 = twinx(ax3)
    ax3.plot(data[:,0],goods_composite_ia/norm1,'b-',label='%d GOODS SNe Ia' %norm1)
    ax3.plot(data[:,0],composite/len(mass),'k',label='All SNe')
    test1 = goods_composite_ia/composite*(len(mass)/norm1)
    ...
    for ia in ias:
        ...
        ii = where((data[:,0]>lbl)&(data[:,0]<=lbu))
        data = data[ii]
        candels_composite_ia += data[:,2]
    norm2=len(ias)-misses
    ax4 = subplot(223)
    ax41 = twinx(ax4)
    ax4.plot(data[:,0],candels_composite_ia/norm2,'b-',label='%d CANDELS SNe Ia' %norm2)
    ax4.plot(data[:,0],composite/len(mass),'k',label='All SNe')
    test2 = candels_composite_ia/composite*(len(mass)/norm2)
    ...
    ax5.plot(data[:,0], composite/len(mass), 'k',label='All SNe')
    ax51.plot(data[:,0],(candels_composite_ia+goods_composite_ia)/composite*(len(mass)/(norm1+norm2)),
    ...
    #show()
    savefig('plot2.png',transparent=True)
