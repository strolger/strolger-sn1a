...
templates = '/Users/strolger/Box/SN_Rates_workdir'
...
def run(redshift, baseline, sens, type=['iip'], dstep=3, dmstep=0.5, dastep=0.5,
    ...
    if redshift > 1.5:
        vega_spec = loadtxt(templates+'/templates/vega_model.dat')
    else:
        vega_spec = loadtxt(templates+'/templates/vega_model_mod.dat')
...
def kcor(f1,f2,models_used_dict,best_age,redshift,vega_spec, extrapolated=True):
    ...
    for model in models_used_dict.keys():
        ...
        idx = where((abs(spec[:,0]-best_age)==min(abs(spec[:,0]-best_age)))&(abs(spec[:,0]-best_age)<3.))
...
def ext_dist_Ia(ext,observed_filter,redshift):
    ...
    Jha = loadtxt(templates+'/templates/Jha_ext.txt')
    Jha[:,0] = Jha[:,0]/A_1*A_2
    p0 = [1.,1.]
...
def ext_dist(ext,observed_filter,redshift,observed=True):
    ...
    if observed:
        if not os.path.isfile(templates+'/templates/ext_model.pkl'):
            f = open(templates+'/templates/ext_model.txt','r')
            lines = f.readlines()
            f.close()
            Av=[]
            for line in lines:
            ...
            Av=array(Av)
            pickle.dump(Av,open(templates+'/templates/ext_model.pkl','wb'))
        else:
            Av = pickle.load(open(templates+'/templates/ext_model.pkl','rb'))
        Av=Av/A_1*A_2
        ...
        HBD = loadtxt(templates+'/templates/HBD_ext.txt')
        HBD[:,0]=HBD[:,0]/A_1*A_2
...
if __name__=='__main__':
    ...
    dvol = cosmocalc.volume(redshift+0.2)-cosmocalc.volume(redshift-0.2)
